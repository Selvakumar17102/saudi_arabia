<?php
	...
	$sql = "SELECT * FROM invoice WHERE date < '$today' AND paystatus!=0 ORDER BY date DESC";
	...
	while($row = $result->fetch_assoc())
	{
		...
		if($divi == $div1)
		{
			...
			{
				$recdate = date('Y-m-d',strtotime($row["recdate"]));
				if($today < $recdate)
				{
					$tot += $row["demo"];
				}
...
<head>
	...
	<!-- PAGE TITLE HERE ============================================= -->
	<title>Consolidate Outstanding Reports | Project Management System</title>
	...
	<main class="ttr-wrapper">
		<div class="container-fluid">
			<div class="db-breadcrumb">
				<h4 class="breadcrumb-title">Consolidate Outstanding Reports</h4>
			</div>
				...
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 m-b30 m-t30">
                    <div class="widget-box">
						<div class="card-header">
							<div class="row">
								<div class="col-sm-8">
									<h4 style="float: left !important"><?php echo ucfirst(strtolower($div2)) ?> - Invoice Outstanding Reports</h4>
								</div>
								<div class="col-sm-4">
                    	        ...
                    	        <table id="dataTableExample" class="table table-striped">
                                    <thead>
                                        <tr>
                                            <th>S.NO</th>
											<th>Project Name</th>
											<th>Outstanging Value (QAR)</th>
                                        </tr>
                                    </thead>
									<tbody>
										<?php
                                            $sql = "SELECT * FROM project WHERE divi='$div1'";
                                            $result = $conn->query($sql);
											$count = 1;
                                            while($row = $result->fetch_assoc())
                                            {
                                                ...
                                                $sql1 = "SELECT * FROM invoice WHERE date < '$today' AND paystatus!=0 AND pid='$pid'";
                                                $result1 = $conn->query($sql1);
                                                while($row1 = $result1->fetch_assoc())
                                                {
                                                    ...
                                                    {
                                                        $recdate = date('Y-m-d',strtotime($row1["recdate"]));
                                                        if($today < $recdate)
                                                        {
                                                            $tot += $row1["demo"];
                                                        }
                                        ...
                                        ?>
                                                    <tr>
                                                        ...
                                                        <td><center><a href="statement-main.php?id=<?php echo $eid ?>&fdate=<?php echo $thismonth ?>&tdate=<?php echo $today ?>"><?php echo $row2["name"] ?></a></center></td>
                                                        <td><center><?php echo number_format($tot) ?></center></td>
                                                    </tr>
                                        <?php
                                                }
											}
...
<script>
	function boss(val)
	{
		if(val != "")
		{
			location.replace("consolidate-outstanding.php?mid=<?php echo $mid ?>&id="+val);
		}
	}
	function search()
	{
		...
		else
		{
			...
			{
				location.replace("consolidate-outstanding.php?mid=<?php echo $mid ?>&fdate="+fdate+"&tdate="+tdate);
			}
		}
	}
</script>
