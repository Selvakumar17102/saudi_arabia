<?php
	...
	$month = date('m');
					...
					while($row7 = $result7->fetch_assoc())
					{
						...
						$sql8 = "SELECT * FROM project WHERE eid='$eid1'";
					...
					$sql8 = "SELECT sum(current) as current FROM invoice WHERE YEARWEEK(recdate) = YEARWEEK(NOW())";
					$result8 = $conn->query($sql8);
					$row8 = $result8->fetch_assoc();
					$weekcollection = $row8["current"];
					if($weekcollection == "")
					{
						$weekcollection = 0;
					}
										
					$sql9 = "SELECT * FROM enquiry WHERE YEARWEEK(qdatec) = YEARWEEK(NOW())";
					...
					$sql11 = "SELECT sum(current) as current FROM invoice WHERE YEAR(recdate) = YEAR(CURDATE())";
					$result11 = $conn->query($sql11);
					$row11 = $result11->fetch_assoc();
					$yearcollection = $row11["current"];
					if($yearcollection == "")
					{
						$yearcollection = 0;
					}
										
					$sql12 = "SELECT * FROM enquiry WHERE YEAR(qdatec) = YEAR(CURDATE())";
					$result12 = $conn->query($sql12);
					while($row12 = $result12->fetch_assoc())
					{
						$eid = $row12["id"];

						$sql13 = "SELECT * FROM project WHERE eid='$eid'";
						$result13 = $conn->query($sql13);
						$row13 = $result13->fetch_assoc();

						 $yearsales += $row13["value"];
					} 
					if($yearsales == "")
					{
						$yearsales = 0;
	...
	<body class="ttr-pinned-sidebar">
		...
		<main class="ttr-wrapper">
			...
			<div class="row head-count">
				...
				<div class="col-md-6 col-lg-3 col-xl-3 col-sm-6 col-12 m-b10">
					<div class="widget-card widget-bg1">					 
						<div class="wc-item">
							...
							<span class="wc-stats">
								Sal. : <?php echo number_format($todaysales, 2); ?> QR <br>Col. : <?php echo number_format($todaycollection, 2); ?> QR 
							</span>		
				...
				<div class="col-md-6 col-lg-3 col-xl-3 col-sm-6 col-12 m-b10">
					<div class="widget-card widget-bg2">					 
						<div class="wc-item">
							...
							<span class="wc-stats">
								Sal. : <?php echo number_format($weeksales, 2); ?> QR <br>Col. : <?php echo number_format($weekcollection, 2); ?> QR 
							</span>		
				...
				<div class="col-md-6 col-lg-3 col-xl-3 col-sm-6 col-12">
					<div class="widget-card widget-bg3">					 
						<div class="wc-item">
							...
							<span class="wc-stats">
								Sal. : <?php echo number_format($monthsales, 2); ?> QR <br>Col. : <?php echo number_format($monthcollection, 2); ?> QR 
							</span>		
				...
				<div class="col-md-6 col-lg-3 col-xl-3 col-sm-6 col-12">
					<div class="widget-card widget-bg4">					 
						<div class="wc-item">
							...
							<span class="wc-stats">
								Sal. : <?php echo number_format($yearsales, 2); ?> QR <br>Col. : <?php echo number_format($yearcollection, 2); ?> QR 
							</span>
						</div>				      
					</div>
				</div>
				...
				<div class="row m-t30">
					<div class="col-lg-12">
						<div class="row">
							...
							<div class="card1">
								<div id="chart_div" style="width: 100%;height: 300px;box-shadow: #0000001a -20px 20px 28px -22px; border: none;border-radius: 8px;"></div>
							</div>
							</div>
							<div class="col-lg-6 m-b30">
							<div class="card1">
								<div id="donutchart" style="width: 100%; height: 300px;box-shadow: #0000001a -20px 20px 28px -22px; border: none;border-radius: 8px;"></div>
							</div>
							</div>
							<div class="col-lg-12 m-b30">
							<div class="card1">
								<div id="curve_chart" style="height:400px;box-shadow: #0000001a -20px 20px 28px -22px; border: none;border-radius: 8px;"></div>
							</div>
							</div>
						</div>
					</div>
      ...
      function drawChart() 
	  {
		...
		<?php
			...
			for($startdate = date('Y-m-01');$startdate <= $enddate;$startdate = date("Y-m-d",strtotime("+1 day", strtotime($startdate))))
			{
				...
				$tempdate = date("d",strtotime($startdate));
