						...
						<div class="widget-inner">
                            <div class="table-responsive">
                    	        <table id="dataTableExample1" class="table table-bordered table-striped table-hover">
									...
									<tbody>
										<?php
                                           $sql = "SELECT * FROM expence_invoice WHERE date BETWEEN '$thismonth' AND '$today' AND type='1'";
                                           echo $sql."<br>";
                                           $result = $conn->query($sql);
                                           while($row = mysqli_fetch_array($result)){
                                                $expence_table_id = $row['id'];
                                                $expence_table_invoice_id = $row['invoice_id'];
                                                
                                                    // INCOME DAY BOOK ENTRY
                                                if($expence_table_invoice_id == null){
                                                    echo "NUll"."<br>";
                                                }
                                                    //PROJECT MANAGEMENT TOOL ENTRY
                                                else{
                                                   $expence_sql = "SELECT * FROM expence_invoice INNER JOIN invoice ON expence_invoice.invoice_id = invoice.id WHERE expence_invoice.id = $expence_table_id";
                                                    $tetst = $conn->query($expence_sql);
                                                    $sadrow = mysqli_fetch_array($tetst);
                                                    echo $sadrow['invid']."<br>";
                                                }
                                           }
...
<script>
	function boss(val)
	{
		if(val != "")
		{
			...
			location.replace("income-account-new.php?id="+val);
		}
	}
	function search()
	{
		...
		else
		{
			...
			else
			{
				...
				location.replace("income-account-new.php?fdate="+fdate+"&tdate="+tdate);
