	...
	if(isset($_POST["pay"]))
	{
		...
		{	
			if($term == 1)
			{
				...
				{
					$sql5 = "INSERT INTO invoice (invid,rfqid,term,total,current,percent,date,po,invdoc) VALUES ('$invid','$rfqid','$term','$total','$pay','$current','$date','$po','$invdoc')";
			...
			else
			{
				...
				{
					$sql5 = "INSERT INTO invoice (invid,rfqid,term,total,current,date,po,invdoc) VALUES ('$invid','$rfqid','$term','$total','$current','$date','$po','$invdoc')";
					if($conn->query($sql5) === TRUE)
					{
						header("location: invoice.php?id=$id&msg=Invoice Generated Successfully!");
					}
...
<head>
	...
	function validate()
	{
		var x = document.getElementById("invid").value;
		var y = document.getElementById("pono").value;
		var z = document.getElementById("current").value;
        
        if(x == "")
		{
			$("#invid").css("border", "1px solid red");
        	return false;
		}
		if(y == "")
		{
					...
					<form class="edit-profile" method="post" enctype="multipart/form-data">
						<div class="">
							...
							<?php
								...
								if($result6->num_rows == 0 && $testing == "TRUE")
								{
							...
							<div class="form-group row" style="margin-top:30px">
								...
								<div class="col-sm-4">
									<input type="text" class="form-control" name="po" id="pono">
								</div>
							...
							<div class="form-group row" style="margin-top:30px">
								...
								<div class="col-sm-3">
									<input type="text" class="form-control" name="date" value="<?php echo date('F') ?>" readonly>
								</div>
								<?php
									}
								...
								<label class="col-sm-2 col-form-label">Current Pay</label>
								<div class="col-sm-4">
					...
					<div class="card">
                        ...
                        ?>
										...
										<td><center><?php echo date('d-m-Y',strtotime($row["date"])) ?></center></td>
											...
											if($row1["pterms"] == 2)
											{
												$test = date(strtotime($row["date"]));
												$lastmonth = date("F", strtotime("-1 months", $test));
												
												echo '<td><center>'.$lastmonth.'</center></td>';
											}
											if($row1["pterms"] == 3)
											{
                        ...
                        ?>
										...
										?>
											...
											<form action="invoice-recieve.php?id=<?php echo $id ?>" method="post">
												...
												<td><button type="submit" class="btn-link" title="Next"><span class="notification-icon dashbg-yellow"><i style="margin-left:0px" class="fa fa-arrow-right" aria-hidden="true"></i></span></button></td>
											</form>
                                        </tr>
