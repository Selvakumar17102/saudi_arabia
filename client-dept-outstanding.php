...
<body class="ttr-pinned-sidebar">
                                ...
                                <table id="dataTableExample1" class="table table-striped">
                                    <thead>
                                        ...
                                        <tr>
                                            <th>S.No</th>
                                            <th>Department</th>
                                            <th>Outstanding Value (SAR)</th>
                                            <th>Outstanding VAT (SAR)</th>
                                            <th>Due Value (SAR)</th>
                                            <th>Due VAT (SAR)</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        $dep_array = array('Engineering', 'Simulation & Analysis', 'Sustainability','Acoustics', 'Laser Scanning','Oil & Gas');
                                        $array_count = count($dep_array);
                                        $over_all = 0;
                                        for ($i = 0; $i < $array_count; $i++) {
                                            $division = $dep_array[$i];
                                            $amo1 = $amo2 = $pend = $tot = $vat_tot = 0;
                                            $sql = "SELECT * FROM project WHERE divi='$division'";
                                            $result = $conn->query($sql);
                                            $count = 1;
                                            $Total_et_value = 0;
                                            $search_collected_value = $total = $search_invoice_value = $total_collected = $total_collected_value = $total_invoice_value = $total_invoice_vat = $total_collected_vat = 0;
                                            $overall_invoice_value = $overall_collected_value = $overall_invoice_vat = $overall_collected_vat = 0;
                                            while ($row = $result->fetch_assoc()) {
                                                $eid = $row["eid"];
                                                $pid = $row["id"];
                                                $scope = $row["scope"];
                                                ...
                                                $sql2 = "SELECT sum(demo) as total_invoice_value FROM invoice WHERE pid='$proid' AND paystatus !='0'";
                                                $result2 = $conn->query($sql2);
                                                $row2 = $result2->fetch_assoc();
                                               ...
                                               $total_invoice_value = $row2['total_invoice_value'];
                                                ...
                                                // vat value
                                                $sql22 = "SELECT sum(demo_gst) as total_invoice_vat FROM invoice WHERE pid='$proid' AND paystatus !='0'";
                                                $result22 = $conn->query($sql22);
                                                $row22 = $result22->fetch_assoc();

                                                $total_invoice_vat = $row22['total_invoice_vat'];
                                                ...
                                                // total collectsd vat
                                                $sql23 = "SELECT sum(current_gst) as total_collected_vat FROM invoice WHERE pid='$proid' AND recdate!='' AND recdate!='0000-00-00'";
                                                $result23 = $conn->query($sql23);
                                                $row23 = $result23->fetch_assoc();

                                                $total_collected_vat = $row23['total_collected_vat'];
                                                ...
                                                $overall_invoice_vat += $total_invoice_vat;

                                                $overall_collected_value += $total_collected_value;
                                                $overall_collected_vat += $total_collected_vat;
                                            ...
                                            ?>
                                                <!-- <tr>
                                                    ...
                                                    <td>
                                                        <center><a href="dept-invoice-report.php?dept=<?php echo $dep_array[$i]; ?>"><?php echo $dep_array[$i]; ?></a></center>
                                                    </td>
                                                    <td ><center><?php echo number_format($overall_invoice_value - $overall_collected_value, 2); ?></center></td>
                                                    <td ><center><?php echo number_format($overall_invoice_vat - $overall_collected_vat,2); ?></center></td>
                                                    <td>
                                                        <center><?php echo number_format($tot, 2); ?></center>
                                                    </td>
                                                    <td>
                                                        <center><?php echo number_format($vat_tot, 2); ?></center>
                                                    </td>
                                                ...
                                                <tr>
                                                    ...
                                                    <td><center><a href="dept-invoice-report.php?dept=<?php echo $dep_array[$i]; ?>"><?php echo $dep_array[$i]; ?></a></center></td>
                                                    <td><center><?php echo number_format($overall_invoice_value - $overall_collected_value, 2); ?></center></td>
                                                    <td><center><?php echo number_format($overall_invoice_vat - $overall_collected_vat,2); ?></center></td>
                                                    <td><center><?php echo number_format($tot, 2); ?></center></td>
                                                    <td><center><?php echo number_format($vat_tot, 2); ?></center></td>
                                                </tr>
                                            <?php
    ...
    <script>
        document.getElementById('total_amt').innerHTML = "<?php echo "SAR " . number_format($total_amt, 2); ?>"
    </script>
    <script>
        // alert("fdgkjhdsgjkl");
        function boss(val) {
